...
class CommunicationController extends Controller {
  ...
  async sendMessage() {
    try {
      const { conversationId, content, type = 'text', replyTo, attachments = [] } = this.req.body;
      const userId = this.req.user.id;
      ...
      for (const participant of otherParticipants) {
        await this.notificationService.send({
          ...
          title: 'New Message',
          message: `You have a new message in ${conversation.title || 'conversation'}`,
          type: 'message',
  ...
  async updateMessage() {
    try {
      ...
      // Emit to WebSocket
      this.webSocketService.emitToConversation(message.conversationId, 'message_updated', {
        messageId: message._id,
        content,
        isEdited: true
      });
  ...
  async deleteMessage() {
    try {
      ...
      // Emit to WebSocket
      this.webSocketService.emitToConversation(message.conversationId, 'message_deleted', {
        messageId: message._id
      });
  ...
  async createConversation() {
    try {
      const { participantIds, type = 'direct', title, description, relatedId, relatedType } = this.req.body;
      const userId = this.req.user.id;
      ...
      allParticipants.forEach(participantId => {
        this.webSocketService.emitToUser(participantId, 'conversation_created', conversation);
      });
  ...
  async updateConversation() {
    try {
      ...
      // Emit to WebSocket
      this.webSocketService.emitToConversation(id, 'conversation_updated', conversation);
  ...
  async searchMessages() {
    try {
      const { query, conversationId, type, page = 1, limit = 20 } = this.req.query;
      const userId = this.req.user.id;
  ...
  async broadcastMessage() {
    try {
      ...
      if (!['SuperAdmin', 'IndustryAdmin', 'Support'].includes(this.req.user.role)) {
        return this.res.status(403).send({
          status: false,
      ...
      if (userIds && userIds.length > 0) {
        ...
        const users = await UserSchema.find({ role: { $in: roles } }).select('_id');
  ...
  async markMessagesAsRead() {
    try {
      ...
      // Emit to WebSocket
      if (conversationId) {
        this.webSocketService.emitToConversation(conversationId, 'messages_read', {
          userId,
          readAt: new Date()
        });
      }
  ...
  async getNotifications() {
    try {
      const { page = 1, limit = 20, unreadOnly = false, type, priority } = this.req.query;
      const userId = this.req.user.id;
  ...
  async updateTrackingData() {
    try {
      ...
      this.webSocketService.emitToRoom(`tracking_${trackingId}`, 'tracking_update', trackingData);
      ...
      if (updateData.status && updateData.status !== trackingData.status) {
        ...
        this.webSocketService.emitToRoom(`entity_${trackingData.entityId}`, 'status_change', {
          trackingId,
  ...
  async createTrackingData() {
    try {
      ...
      if (!trackingDataInput.trackingId) {
        trackingDataInput.trackingId = `TRK-${Date.now()}-${Math.random().toString(36).substr(2, 5).toUpperCase()}`;
      }
      ...
      // Emit to WebSocket
      this.webSocketService.emitToRoom(`entity_${trackingData.entityId}`, 'tracking_created', trackingData);
