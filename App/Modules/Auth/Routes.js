...
module.exports = (app, express) => {
  ...
  router.get("/auth/google", (req, res) => {
    const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${config.googleClientId}&redirect_uri=${config.googleRedirectUri}&scope=openid%20email%20profile&response_type=code&access_type=offline&prompt=consent`;
    res.redirect(googleAuthUrl);
  });

  // ---------------- LinkedIn OAuth ----------------
  router.get("/auth/linkedin", (req, res) => {
    const linkedinAuthUrl = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${config.linkedinClientId}&redirect_uri=${config.linkedinRedirectUri}&scope=r_liteprofile%20r_emailaddress`;
    res.redirect(linkedinAuthUrl);
  });

  // ---------------- Microsoft OAuth ----------------
  router.get("/auth/microsoft", (req, res) => {
    const microsoftAuthUrl = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${config.microsoftClientId}&response_type=code&redirect_uri=${config.microsoftRedirectUri}&scope=openid%20profile%20email&response_mode=query`;
  ...
  // Logout
  router.post("/auth/logout", Middleware.isUserAuthorized, (req, res) => {
    const controller = new AuthController().boot(req, res);
  ...
  // Get Current User
  router.get("/auth/me", Middleware.isUserAuthorized, (req, res) => {
    const controller = new AuthController().boot(req, res);
  ...
  router.get("/auth/permissions", Middleware.isUserAuthorized, (req, res) => {
    const controller = new AuthController().boot(req, res);
  ...
  router.put(
    "/auth/company/members/:memberId/role",
    Middleware.isUserAuthorized,
