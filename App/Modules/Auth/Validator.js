      ...
      try {
        req.schema = Joi.object().keys({
          ...
          phone: Joi.string()
            ...
            .required(),
          role: Joi.string()
            .valid("IndustryAdmin", "IndustryMember", "Professional", "Vendor")
            .required(),
  ...
  static validateChangePassword() {
    return async (req, res, next) => {
      try {
        ...
        next(error); // better than throw, to pass error to Express handler
      }
    };
  }
  ...
  static validateSocialCallback() {
    return async (req, res, next) => {
      try {
        req.schema = Joi.object().keys({
          provider: Joi.string()
            .valid("google", "linkedin", "microsoft")
            .required(),
  ...
  static validateMemberRoleUpdate() {
    return async (req, res, next) => {
      try {
        req.schema = Joi.object().keys({
          role: Joi.string()
            .valid("IndustryMember", "IndustryAdmin")
            .required(),
          reason: Joi.string().max(500).optional(),
        });
        next();
  ...
  static validateUserApproval() {
    return async (req, res, next) => {
      try {
        req.schema = Joi.object().keys({
          action: Joi.string()
            .valid("approve", "request_documents", "move_to_review")
            .required(),
  ...
  static validateSessionManagement() {
    return async (req, res, next) => {
      try {
        req.schema = Joi.object().keys({
          action: Joi.string()
            .valid("terminate_all", "terminate_session")
            .required(),
    ...
    return async (req, res, next) => {
      try {
        req.schema = Joi.object().keys({
          password: Joi.string().required(),
          reason: Joi.string()
            .valid("temporary", "permanent", "security", "other")
            .required(),
          feedback: Joi.string().max(1000).optional(),
          dataRetention: Joi.string()
            .valid("delete_immediately", "retain_30_days", "retain_90_days")
            .default("retain_30_days"),
        });
        next();
      } catch (error) {
        throw new Error(error);
